...
def main() -> int:
    if not 2 <= len(sys.argv) <= 3:
        return show_error("Usage: parse_install_yaml_cucr install.yaml [--now]")
...
def installyaml_parser(path: str, now: bool = False) -> Mapping:
    ...
    for install_item in install_items:
        ...
        try:
            ...
            if install_type == "empty":
                ...
                if source_type == "git":
                    command = f"cucr-install-ros {catkin_git(source)}"
                elif source_type == "system":
                    system_packages.append(f"ros-{ros_release}-{source['name']}")
                    command = f"cucr-install-ros system {source['name']}"
                else:
            ...
            # Non ros targets that are packaged to be built with catkin
                ...
                command = f"cucr-install-git {type_git(install_item)}"
                ...
                command = f"cucr-install-{install_type} {pkg_name}"
    ...
    for install_type, pkg_list in now_cache.items():
        if pkg_list:
            ...
            command = f"cucr-install-{install_type} {pkg_list}"
            commands_append(command)
